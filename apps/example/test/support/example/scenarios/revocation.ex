defmodule Example.Scenarios.Revocation do
  ...
  def rev_01 do
    ...
    assert Mediate.Test.poll(fn -> not reads?(subject("ann"), document) end)
  ...
  def rev_06 do
    ...
    {removed, revoked} = Mediate.Test.changes(fn -> Accounts.unassign("ann", world.program.id) end)
    assert removed == 1
  ...
  # A revoke takes the assignment away and leaves the document and the
  # program where they were.
  defp assert_kept(world, document) do
    assert %Document{} = Repo.get(Document, document.id, mediate: Fixture.exemption())
    assert %Program{} = Repo.get(Program, world.program.id, mediate: Fixture.exemption())
  end
