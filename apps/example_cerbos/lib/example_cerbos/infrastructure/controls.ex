defmodule ExampleCerbos.Infrastructure.Controls do
  ...
  # does not carry: which controls are effective on a marking.
  #
  # A control is effective when the marking declares it, or when a specified
  # category the marking names implies it (C3). The derivation is a union
  # over the control names, one query per name. Each query selects the row
  # and the name as text. A portion is under control while its document is
  # (C4, C5). The date is on a row the portion does not carry, so the moment
  ...
  @doc """
  The controls effective on each portion's own marking, while the document's
  decontrol date is absent or after `at`.
  """
  ...
  # carries no date of its own. A control the document has released is
