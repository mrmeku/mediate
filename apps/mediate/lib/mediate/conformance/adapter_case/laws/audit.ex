defmodule Mediate.Conformance.AdapterCase.Laws.Audit do
  @moduledoc """
  ...
  def denial_reason(%{case: %{world: module}} = context) do
    ...
    {answer, events} = Laws.recorded(fn -> Mediate.authorize({:privileged, account}, operation, object) end)
  ...
  def scoped_verdicts(%{case: %{world: module}} = context) do
    ...
    {{scope, review}, events} =
      Laws.recorded(fn ->
        {Mediate.scope(subject, operation, type), Mediate.review(subject, [subject], operation, type)}
      end)
  ...
  def access_event(%{repo: repo, case: %{world: module}} = context) do
    ...
    {:ok, %Decision{} = decision} = Mediate.authorize(subject, operation, object)

    {read, events} = Test.accesses(fn -> repo.get(schema, id, mediate: decision) end)
  ...
  def one_operation(%{case: %{world: module}} = context) do
    ...
    operation_id = Mediate.Id.new()

    {{:ok, decision}, [decided]} =
      Laws.recorded(fn -> Mediate.authorize(subject, operation, object, operation_id: operation_id) end)
  ...
  @doc "`au12-01`: the seam publishes the change event of a grant written while the repo is in the write's transaction."
  ...
  def bulk_refused(%{repo: repo, case: %{world: module}} = context) do
    ...
    counted = repo.aggregate(schema, :count, mediate: exemption)

    calls = [
      {:update_all, [schema, [set: [{column, value}]], [mediate: exemption]]},
      {:delete_all, [schema, [mediate: exemption]]},
      {:insert_all, [schema, [%{column => value}], [mediate: exemption]]}
    ]

    {:ok, events} = Test.changes(fn -> Enum.each(calls, &assert_bulk(repo, &1)) end)
    assert events == []
    assert repo.aggregate(schema, :count, mediate: exemption) == counted
  end
  ...
  def around_seam(%{repo: repo, case: %{world: module}} = context) do
    ...
    {_result, events} = Test.changes(fn -> repo.query!(statement, [id], mediate: exemption) end)
    assert events == []
    assert repo.get(schema, id, mediate: exemption) == nil
  end
  ...
  def mediated_reads(%{repo: repo, case: %{world: module}} = context) do
    ...
    schema = schema_of(module, type)
    {:ok, %Decision{} = decision} = Mediate.authorize(subject, operation, object)
    reads = reads(repo, schema, repo.get!(schema, id, mediate: decision))
  ...
  defp under_decision(%{repo: repo, case: %{world: module}}, world, decision) do
    ...
    {_read, [accessed]} = Test.accesses(fn -> repo.get(schema_of(module, type), id, mediate: decision) end)
    [changed, accessed]
  end
  ...
  # Each read of the schema under a mediation, with its activity. The
  # third element maps what the read answered to the ids its access event
  # has to name. The stream runs inside a transaction, as Ecto asks. Its
  # one event names no ids.
  defp reads(repo, schema, %{__struct__: schema} = row) do
    ...
    [
      {fn mediation -> repo.get(schema, id, mediate: mediation) end, :read, one},
      {fn mediation -> repo.all(schema, mediate: mediation) end, :query, each},
      {fn mediation -> repo.exists?(schema, mediate: mediation) end, :read, none},
      {fn mediation -> repo.aggregate(schema, :count, mediate: mediation) end, :query, none},
      {fn mediation -> repo.transaction(fn -> repo.stream(schema, mediate: mediation) end) end, :query, none},
      {fn mediation -> repo.reload(row, mediate: mediation) end, :read, one}
    ]
  end
  ...
  defp four_decisions(subject, privileged, operation, object) do
    assert {:ok, %Decision{}} = Mediate.authorize(subject, operation, object)
    assert Mediate.check(subject, operation, object)
    refute Mediate.check(privileged, operation, object)
    assert {:error, %Error{}} = Mediate.authorize(privileged, operation, object)
  end

  defp three_decisions(subject, privileged, operation, {type, _id} = object) do
    assert {:ok, %Decision{}} = Mediate.authorize(subject, operation, object)
    refute Mediate.check(privileged, operation, object)
    assert {_rule, %Decision{verdict: :scoped}} = Mediate.scope(subject, operation, type)
  end
