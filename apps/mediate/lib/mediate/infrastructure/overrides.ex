defmodule Mediate.Infrastructure.Overrides do
  ...
  defmacro __before_compile__(env) do
    generate(env.module, Module.get_attribute(env.module, :mediate_surface))
  end

  @doc """
  The quoted overrides for the functions of `surface` that `module` defines.
  The surface arrives from the repo's own `@mediate_surface`, which
  `use Mediate.Repo` sets. So this module does not read the surface, and a
  ...
  # A lower arity fills the absent arguments with `[]` and calls the full one.
