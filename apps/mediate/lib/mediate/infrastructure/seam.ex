defmodule Mediate.Infrastructure.Seam do
  # What every override `use Mediate.Repo` defines calls. One function per
  # bucket of `Mediate.Infrastructure.Surface`:
  #
  # - `query/5` for the query bucket
  # - `bulk/5` for `update_all` and `delete_all`
  # - `write/5` and `write_all/5` for the write bucket
  # - `raw/4` for raw SQL
  # - `prepare/4` behind `prepare_query/3`, which judges every query, the
  #   ones `preload` generates included
  #
  # A write to an audited schema on an application-role repo runs in a
  # transaction. The seam publishes the change inside that transaction, as
  # `Mediate.Change`, so a handler that writes from the same repository
  # joins it. A read of a protected schema under a decision publishes what
  # it returned, as `Mediate.Access`, after it returns. The owner-role repo
  # records nothing, because it is the library's own channel.
  @moduledoc false
  ...
  alias Mediate.Domain.Matching
  alias Mediate.Domain.Mediation
  alias Mediate.Domain.Source
  alias Mediate.Error
  alias Mediate.Id
  ...
  read under a decision returned. `prepare/4` judges the query itself.
  """
  ...
  def raw(repo, {name, arity} = call, opts, continue) when is_atom(repo) and is_list(opts) do
    case Option.resolve(repo, call, nil, opts) do
      {%Mediation{exemption: %Mediate.Exemption{}}, opts} ->
        continue.(opts)

      {%Mediation{decision: %Decision{}}, _opts} ->
        raise Error.invalid(:mediate, "Repo.#{name}/#{arity} takes an exemption, not a decision")
  ...
  def prepare(repo, operation, %Ecto.Query{} = query, opts) when is_atom(repo) and is_list(opts) do
    case Keyword.get(opts, :mediate) do
      %Mediation{} = mediation ->
  ...
  # The module a refusal names as the one that made the call. The seam
  # reads the stack only where there is a refusal to name.
  defp caller(repo), do: fn -> Caller.module(repo) end

  # Calls the adapter's `around_query/3` when a decision is in force and the
  # adapter defines it. Otherwise it runs the call.
  ...
  # The event a consumer builds a record from. The seam publishes it inside
  # the transaction the write runs in. The row the caller loaded is the
  ...
  # it publishes commit together. A write that reports an error rolls the
  # transaction back, and the error comes out as the write returned it.
  ...
  defp audited?(repo, schema), do: repo.__mediate__(:role) == :app and Schema.audited?(schema)
  ...
  # The seam refuses a bulk write to an audited schema. One statement
  # changes many rows, and the seam cannot read back the change each row
  # made. So no caller of an application-role repo can make one. The
  # owner-role repo is the library's own channel and records nothing, so it
  # passes.
  defp refuse_bulk(repo, {name, arity}, root) do
    schema = schema_of(root)

    if repo.__mediate__(:role) != :owner and Schema.audited?(schema) do
      raise Error.invalid(
              :bulk_write,
