defmodule Mediate.Port do
  ...
  """
  @spec scope(Mediate.subject(), atom(), atom(), options()) :: {Ecto.Query.dynamic_expr(), Decision.t()}
  ...
  """
  @spec review(Mediate.subject(), [Mediate.subject()], atom(), atom(), options()) :: reviewed()
  ...
  # its place. The port denies an unknown subject kind before it asks the
  # adapter, and an engine error after.
  ...
  # error is the engine error the adapter gave or the exception the adapter
  # raised. The third element carries the exception, so the event names
  # what broke. A decider that raises closes the door and never reaches
  # the caller. So no decider carries a rescue clause for the driver
  # underneath it.
  ...
  # scope call that is the rule in the object's place.
