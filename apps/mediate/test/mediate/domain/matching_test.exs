defmodule Mediate.Domain.MatchingTest do
  ...
  test "a query from a table name has no root schema and passes without a mediation" do
    query = from(a in "mediate_fixture_accounts", select: a.id)
  ...
  test "a join to a subquery is judged, and a subquery over a table name admits nothing" do
    protected = from(a in "mediate_fixture_accounts", join: f in subquery(from(f in Folder, select: f.id)), on: true)
    ...
    schemaless =
      from(a in "mediate_fixture_accounts",
        join: t in subquery(from(t in "mediate_fixture_items", select: t.id)),
        on: true
      )
  ...
  test "a source with no schema is judged by nothing it reaches: a fragment, and an association off one" do
    ...
    assoc = from(a in "mediate_fixture_accounts", join: f in assoc(a, :folder), select: a.id)
