defmodule Mediate.Domain.SourceTest do
  ...
  test "root/1 answers the schema, table name, or nil for every shape the Repo takes" do
    ...
    assert Source.root("mediate_fixture_folders") == "mediate_fixture_folders"
    assert Source.root({"mediate_fixture_folders", Folder}) == Folder
    folders = from(f in Folder, select: f.id)
    items = from(t in "mediate_fixture_items", select: t.id)
    ...
    assert Source.root(items) == "mediate_fixture_items"
    assert Source.root(over_items) == "mediate_fixture_items"
