defmodule Mediate.PortTest do
  ...
  test "a call publishes one decision event carrying who asked, what was answered, and how long it took" do
    ...
    assert_received {[:mediate, :decision], _ref, %{duration: duration}, metadata}
    assert duration >= 0
    ...
    assert_received {[:mediate, :decision], _ref, _measurements, %{verdict: :deny, subject_kind: :non_person_entity}}
  end
  ...
  test "a narrowing call publishes the rule in the object's place" do
    ...
    assert_received {[:mediate, :decision], _ref, _measurements, %{verdict: :scoped, object: rule}}
    assert %DynamicExpr{} = rule
  end

  test "a decider that raises denies closed, and its event carries the exception" do
    :ok = Mediate.Test.with_config(adapter: Raising)
  ...
  test "the operation id given is the one every record of the call carries" do
    ...
    assert_received {[:mediate, :decision], _ref, _measurements, %{operation_id: ^id}}
