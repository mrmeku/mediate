defmodule Mediate.Cerbos.Client do
  ...
  uses, and one telemetry event per call.

  The transport is `httpc`, which OTP ships, and the encoder is Elixir's
  `JSON`. So this adapter adds no dependency to an application that takes
  it. Every call is a `POST` or a `GET` to the address the configuration
  entry names, with a deadline. A sidecar that does not answer within the
  ...
  @timeout 5_000
  @telemetry [:mediate, :cerbos, :request]
