defmodule Mediate.Cerbos.Infrastructure.Values do
  ...
  # library caller. One query reads the columns of a kind. One query per
  # subquery attribute reads its values, built from the subject and the
  # environment the port stamped the request with.
  #
  # Every declared attribute goes to the sidecar, whether or not the row
  ...
  # sidecar record an evaluation error against the request, and not read
  # the attribute as empty. That record carries a fault where the answer is
  # sound.
  #
  ...
  # name `Mediate.Cerbos.Attribute.reserved/0`. `environment/2` builds them.
  # They hold the moment the port stamped the request with, always. They
  # hold each fact an `environment` block declared, whether the caller
  # supplied it or not.
  # The codec cuts every moment among them to the second, and says why.
  ...
  alias Mediate.Cerbos.Infrastructure.Codec
  ...
  @spec principal(Binding.t(), Mediate.subject(), Mediate.environment()) ::
  ...
  @spec environment(Binding.t(), Mediate.environment()) :: Attribute.values()
  ...
  @spec resources(Binding.t(), Mediate.subject(), atom(), [Mediate.object()], Mediate.environment()) ::
  ...
  @spec of(Binding.t(), Mediate.subject(), atom(), [term()], Mediate.environment()) ::
          {:ok, %{String.t() => Attribute.values()}} | {:error, String.t()}
