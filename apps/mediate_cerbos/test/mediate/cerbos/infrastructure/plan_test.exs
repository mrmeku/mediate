defmodule Mediate.Cerbos.PlanTest do
  ...
  test "a comparison with nothing is a null test, and an ordering against nothing is no rule", ctx do
    Sandboxed.insert!(%Item{id: 11, title: "item 11"}, mediate: World.exemption())
  ...
  test "a value the column cannot hold and a column the schema does not hold are errors", ctx do
    ...
    assert absent =~ "the plan reads nowhere, which Mediate.Fixture.Folder does not hold"
  end
  ...
  test "a plan this adapter cannot express emits the fallback event and fails", ctx do
    ...
    assert {:error, %Error{reason: :engine_unreachable} = error} =
             Mediate.Cerbos.scope(ctx.ann, :share, :folder, ctx.environment, address: ctx.address)

    assert error.detail =~ "Mediate.Cerbos failed during scope"
  ...
  defp items(ctx, operator, operands) do
    ...
    Sandboxed.all(query, mediate: World.exemption())
  end
