defmodule Mediate.Cerbos.ValuesTest do
  ...
  defmodule Pair do
    ...
    schema "mediate_cerbos_values_test_pairs" do
  ...
  setup tags do
    ...
    :ok = Test.with_config(adapter: {Mediate.Cerbos, address: Dev.Cerbos.info().address})
  ...
  test "a subquery reads the environment, so a grant of another kind or past its expiry selects nothing", ctx do
    ...
    membership = Sandboxed.get_by!(Membership, [account_id: "ann", folder_id: 1], mediate: World.exemption())
    changeset = Ecto.Changeset.change(membership, expires_at: ~U[2999-01-01 00:00:00Z])
    _updated = Sandboxed.update!(changeset, mediate: World.exemption())
  ...
  test "a column the row holds as an atom reaches the sidecar as the text the column holds", ctx do
    [%Membership{id: id}] = Sandboxed.all(Membership, mediate: World.exemption())
