defmodule Mediate.Conformance.RepoCase.Rows do
  @moduledoc """
  ...
  - a row of one audited schema
  - a change to one of its fact fields
  - the mediation those writes carry
  - the way this deployment writes the same row without the seam
  - a row of one protected schema, with a decision that admits a read of it
  ...
  @doc "An unwritten row of an audited schema. Each call answers one the case can write beside the ones before it."
  @callback row() :: struct()
  ...
  @doc "The `mediate:` option those writes carry: a decision that admits the schema, or a declared exemption."
  @callback mediation() :: term()

  @doc "Change a fact field of the written row without the seam, as a patch applied by hand does."
  ...
  @doc "A decision from `Mediate.authorize/4` that admits a read of the written row of `protected/0`."
  @callback decision(struct()) :: Mediate.Decision.t()
