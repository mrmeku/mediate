defmodule Mediate.Conformance.Fixture.World do
  ...
  with or without clearance, folders, items in folders, and memberships of
  accounts on folders. Each membership has a role, the kind of subject that
  holds it, and an expiry.

  Every adapter's conformance artifact for the fixture encodes the rule:

  - `:read` on a folder needs a live membership on it that the kind that asks
    holds
  - `:edit` needs such a membership with the editor role
  - an item answers as its folder does
  - an account without clearance gets a denial for everything

  A membership is live when it names no expiry or one after the moment the
  configured clock reads. `allowed?/4` reads the moment from that clock.
  ...
  @behaviour Mediate.Conformance.World
  ...
  @typedoc "What a membership holds: the role, the kind of subject that holds it, and when it expires."
  @type membership :: %{role: :reader | :editor, kind: Mediate.subject_kind(), expires_at: DateTime.t() | nil}

  @typedoc "Account id to clearance, the folder ids, item id to its folder, and `{account, folder}` to its membership."
  @type t :: %__MODULE__{
  ...
  @impl World
  @spec object_of(pos_integer()) :: Mediate.object()
  ...
  @doc "One cleared account, one folder, and one editor membership on it with no expiry, which a user holds."
  @impl World
  @spec granted() :: t()
  def granted do
  ...
  @doc "A membership of the role with no expiry, which a user holds."
  ...
  @impl World
  @spec allowed?(t(), Mediate.subject(), atom(), Mediate.object()) :: boolean()
  ...
  under every kind the port knows. A membership names the kind that holds
  it, and the generators ask under any kind.
  """
  @spec grants(t()) :: [{Mediate.subject(), atom(), Mediate.object()}]
  ...
  def insert(repo, %__MODULE__{} = world) when is_atom(repo) do
    world
    |> rows()
    |> Enum.each(&repo.insert!(&1, mediate: @exemption))
  end
  ...
  def clear(repo) when is_atom(repo) do
    ...
    {_count, nil} = repo.delete_all(Item, mediate: @exemption)
    {_count, nil} = repo.delete_all(Folder, mediate: @exemption)
    :ok
  end

  @doc "Remove the account's membership on the folder, if any, and return the world."
  @impl World
  @spec revoke(module(), t(), Mediate.subject(), pos_integer()) :: t()
  def revoke(repo, %__MODULE__{} = world, {_kind, account}, folder) when is_atom(repo) do
    case repo.get_by(Membership, [account_id: account, folder_id: folder], mediate: @exemption) do
      nil -> :ok
      %Membership{} = membership -> repo.delete!(membership, mediate: @exemption)
    end
  ...
  @doc """
  Insert one membership through the seam and nothing else. The subject's
  kind holds the role, with the attributes given. `expires_at` is the one
  the rule reads. It answers the world it leaves.
  """
  @impl World
  @spec insert_grant(module(), t(), Mediate.subject(), pos_integer(), keyword()) :: t()
  def insert_grant(repo, %__MODULE__{} = world, {kind, account}, folder, attributes)
    ...
    _inserted = repo.insert!(row, mediate: Keyword.get(attributes, :mediation, @exemption))
  ...
  @impl World
  @spec disqualify(module(), t(), Mediate.subject()) :: t()
  def disqualify(repo, %__MODULE__{} = world, {_kind, account}) when is_atom(repo) do
    changeset = Ecto.Changeset.change(repo.get!(Account, account, mediate: @exemption), clearance: nil)
    _updated = repo.update!(changeset, mediate: @exemption)

    %{world | accounts: Map.put(world.accounts, account, nil)}
  end

  @doc "Bring the folder table up to that many rows. No grant covers the added ones."
  @impl World
  ...
  def fill(repo, %__MODULE__{} = world, count) when is_atom(repo) and is_integer(count) do
    ...
    {_written, nil} = repo.insert_all(Folder, rows, mediate: @exemption)
    :ok
  end
