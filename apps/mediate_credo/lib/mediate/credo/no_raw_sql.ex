if Code.ensure_loaded?(Credo.Check) do
  defmodule Mediate.Credo.NoRawSQL do
    @moduledoc false
    use Credo.Check,
      id: "TS0001",
      base_priority: :high,
      ...
      explanations: [
        ...
        `use Mediate.Repo`. That repo checks the decision or the exemption
        that covers the query. `Ecto.Adapters.SQL.query/4`, its variants, and
        any `Postgrex` call reach the database around the seam. So nothing
        checks them.

        Name a module the seam itself rests on, such as an outbox's writer or
        a test cluster's bootstrap, in `allow`.
        """,
