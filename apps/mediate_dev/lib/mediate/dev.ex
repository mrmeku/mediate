defmodule Mediate.Dev do
  ...
  The modules under this name raise the ephemeral cluster, set up the
  sandbox, dump a schema, and start the engines the suites ask for. This
  module holds the wait they share. No published package depends on this
  package outside its test environment.

  A published adapter carries no dependency its own users have no use for.
  To start an operating-system process takes `muontrap`. So the engine
  launchers live here and not in the adapter packages.
  """
