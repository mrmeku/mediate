defmodule Mediate.Dev.Cerbos do
  ...
  sidecar outlives the run that started it.
  ...
  the adapter's configuration entry. `test_helper.exs` calls `start_shared/1`
  once for the run, and a second call returns the first sidecar.

  `start_supervised!/1` is the same server owned by one test, through the
  supervisor of `ExUnit`. A test that publishes a policy version needs it,
  ...
  speaks the policy language or the decision API. The adapter package does
  that.
  """
  ...
  @doc "A sidecar of the current test's own. It stops when the test ends."
  ...
  def info do
    case :persistent_term.get(__MODULE__, nil) do
      ...
      nil -> raise "no shared sidecar; call Mediate.Dev.Cerbos.start_shared/1 from test_helper.exs"
    end
  end
  ...
  # for a listener that asks for port zero, and the sidecar takes it after
  ...
  defp await!(%__MODULE__{} = sidecar, timeout) do
    Mediate.Dev.poll(fn -> healthy?(sidecar.address) end, timeout)
    sidecar
  end
