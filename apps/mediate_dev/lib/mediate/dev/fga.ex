defmodule Mediate.Dev.Fga do
  @moduledoc """
  One `openfga run` per test run, and one of its own for a test that needs a
  server it can throw away.

  `start_shared/1` starts the server under `MuonTrap.Daemon`. The daemon
  kills the operating-system process when the Erlang process that owns it
  dies and when the virtual machine exits. So no server outlives the run
  that started it.
  ...
  each test creates its own. `test_helper.exs` calls `start_shared/1` once
  for the run, and a second call returns the first server.
  ...
  here speaks the model language or the decision API. The adapter package
  does that.
  """
  ...
  def info do
    case :persistent_term.get(__MODULE__, nil) do
      ...
      nil -> raise "no shared server; call Mediate.Dev.Fga.start_shared/1 from test_helper.exs"
    end
  end
  ...
  defp await!(%__MODULE__{} = server, timeout) do
    Mediate.Dev.poll(fn -> healthy?(server.address) end, timeout)
    server
  end
