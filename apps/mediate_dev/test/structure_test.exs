defmodule StructureTest do
  ...
  names a module the file defines, and the file names every other module it
  defines under that one. A reader who knows a module's name knows which
  file holds it. A reader who opens a file knows which module to expect.

  The roots are each package's `lib` and `test/support`, the two trees the
  compiler reads. A path names a module when the module's name, underscored,
  is the path under its root without its extension. A task file follows the
  Mix convention. The dots in the last segment stand for the dots in the
  ...
  - A module under `domain/` names no module under `application/` or
    `infrastructure/`.
  - A module under `infrastructure/` names no module under `application/`.
    So the calls inside a package run one way.
  - In a published package a module under any of the three places carries
    `@moduledoc false`, because none of them is the interface. The example
    and the thin applications keep their docs, because their layers are
    what a reader opens them for.

  The test reads the syntax tree and not the compiled modules. So it starts
  nothing and depends on nothing it reads. The `boundary` library cannot see
  a call to Elixir's own modules or to Erlang. That is why this test holds
  ...
  # a process, a file, a clock, a table, a node, or the log.
  @world ~w(
  ...
  # same number on any node and on any release, so a module that decides can
  # take one.
  @pure [{:erlang, :phash2}]
