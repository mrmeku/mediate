defmodule Mediate.Fga.Client.ListObjects do
  @moduledoc """
  Every object of one type the user holds one relation on. The answer is a
  list of objects. That is what makes a scope one query on the
  application's side: the ids go into the rule the port hands back.

  The server caps how many objects it answers with. So a caller that cares
  whether the list is whole compares its length against the cap it
  declared.
  """
