defmodule Mediate.Fga.Infrastructure.Decide do
  @moduledoc false
  # The answers the adapter gives. An operation becomes a relation. A
  # subject and an object become the two ends of a tuple. The server
  # answers whether a path joins them.
  #
  # An operation is the relation `can_` and its name. That is the one
  ...
  # denial is a denial by default. A graph has no rule that denies, it has
  # no path, and a rule named as the one that denied does not exist.
  #
  # The environment is the context every call carries, which the server
  # evaluates a condition on a tuple against. It holds the caller's facts
  # under their own names. It holds the moment of the call under
  # `current_time`, and the kind of the subject that asks under
  # `subject_kind`. Those two names this package reserves.
  #
  # So a condition
  # that compares a date on a tuple with the present needs nothing of the
  # caller. The same holds for one that compares the kind on a tuple with
  # the kind that asks. The user string stays kind-blind.
  #
  # An entry is where the question goes and what it asks under: the client,
  # the endpoint, the store, the model, and that context. Every answer
  # carries the model as its policy version. The consistency of each call is
  # a constant of this package. A decision asks for the higher consistency,
  # so it misses no tuple a drain has written. A scope asks for the lower
  # one, since a scope is a filter over rows the caller reads anyway.
  #
  # A scope is one `ListObjects`. Under the cap its identifiers become the
  # rule. At the cap or above it the answer is short of the truth and says
  # nothing about it. So this module emits `fallback_event/0` with the level
  # `limited` and fails, and the caller asks per page instead.
  #
  ...
  # refuses. This module answers that refusal as it stands and turns it into
  # no denial of its own. Nothing here can tell an operation no one declared
  # from a model someone published wrong. Either way the caller denies.
  ...
  @fallback [:mediate, :fga, :scope_fallback]
  @guard "guard"
  @scope_cap 1_000
  @time "current_time"
  @kind "subject_kind"
  ...
  @doc "The name the kind of the subject that asks carries in the context of every question."
  ...
  @spec one(entry(), Mediate.subject(), atom(), Mediate.object()) :: {:ok, Answer.t()} | {:error, Error.t()}
  ...
  @doc """
  The rule for an object type: the identifiers `ListObjects` answers with,
  as a `dynamic` over the rows of that type. An answer at the cap or above
  it emits `fallback_event/0` and fails. That is the scope the caller asks
  per page instead.
  """
  @spec scoped(entry(), Mediate.subject(), atom(), atom()) ::
          {:ok, Mediate.Adapter.scoped()} | {:error, Error.t()}
  ...
  @spec refused_scope(entry()) :: Mediate.Adapter.scoped()
  ...
  # At the cap the answer is a page of a longer list, and the server says
