defmodule Mediate.Fga.Model do
  @moduledoc """
  ...
  its models as a tree of usersets instead. The pinned server (1.19.0)
  ships no command that turns one into the other, so the translation is
  here. `compile/1` answers the map `c:Mediate.Fga.Client.write_model/3`
  sends, and the text stays the thing under review.

  What the text can say. A `model` line, a `schema 1.1` line, then `type`
  blocks and `condition` blocks. A type block holds a `relations` line and
  one `define` per relation. A definition is one of these:

  - a direct list: `[user]`, `[user:*]`, `[group#member]`,
    `[user with while_cleared]`
  - a relation on the same object: `editor`
  - a relation reached through another: `can_read from folder`
  - those combined with `or`, `and`, and `but not`

  Operators of two kinds in one definition need parentheses, which is the
  server's rule as well. A definition that mixes them without parentheses
  is an error and not a guess. The compiler reads no comment, because `#`
  names a relation inside a direct list.

  A relation's direct list becomes the entry the server keeps as metadata,
  and the rest of the definition becomes its userset. That is why a
  definition of `[user] or lead` says `this` in the tree and names `user`
  in the metadata.
  """
  ...
  @doc "The parameter type names a condition can declare, by the word the text uses."
  ...
  @doc "`compile/1`, and raises on error."
  ...
  @doc "`read/1`, and raises on error."
  @spec read!(Path.t()) :: t()
  def read!(path) when is_binary(path) do
    case read(path) do
      {:ok, model} -> model
  ...
  # A condition block ends at the line its `}` is on. That is the first
  # line itself where the whole condition sits on one line.
