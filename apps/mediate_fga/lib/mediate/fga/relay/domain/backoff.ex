defmodule Mediate.Fga.Relay.Domain.Backoff do
  @moduledoc false
  # How long the runner waits before its next pass, and how many failures in
  # a row it has counted.
  #
  # A full batch means the rows that follow it are already there, so the
  # next pass runs at once. A pass that delivered less than a batch, and
  # one that stepped aside because another node held the lock, wait the
  # idle interval. A failure waits the backoff, doubled once for each
  # failure that came before it and capped. So the runner asks a database
  # that is down about every half minute and not every second. One success
  # puts the count back to zero.
  #
  # The cap is at sixteen failures as well as at the longest wait. So the
  # number this arithmetic works on stays small however long an outage
  # lasts.
