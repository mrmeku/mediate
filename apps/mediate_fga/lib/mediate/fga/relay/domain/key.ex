defmodule Mediate.Fga.Relay.Domain.Key do
  @moduledoc false
  ...
  # integer stands for this library and the second for the runner. Both
  # come from `:erlang.phash2/1`, which answers the same number for the same
  # term on every node and every release. So two nodes that run one runner
  # arrive at one key and agree on nothing else.
  #
  # `phash2/1` answers below 2^27, which is inside the signed 32-bit integer
  # Postgres takes for each half of the pair.

  @class :erlang.phash2(:mediate_relay)
