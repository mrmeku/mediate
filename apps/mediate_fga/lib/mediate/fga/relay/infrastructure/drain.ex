defmodule Mediate.Fga.Relay.Infrastructure.Drain do
  ...
  # happens inside one transaction. So what the cursor says the pass
  # delivered is what a delivery that returned put out. A delivery that did
  # not return leaves the cursor where it was.
  #
  ...
  # value of the pass. `attempted/1` turns an exception into the same kind
  # of value and lets none travel. This is where a runner meets the
  # database, and a database that is unreachable is a pass that failed, not
  # a process that must die. The pass carries the term and never reads it,
  # so it names no driver.
  #
  # The moment on the pass comes from the clock in the runner's options.
  # `finished/4` reads it once the transaction has returned. So a test that
  # stubs the clock reads its own time, and nothing here asks the system
  # for one.
  @moduledoc false
  ...
  @event [:mediate, :relay, :pass]

  @doc "The event every pass emits, whether it delivered, stepped aside, or failed."
  @spec event() :: [atom()]
  def event, do: @event

  @doc "Runs one pass with validated options and answers what it did."
