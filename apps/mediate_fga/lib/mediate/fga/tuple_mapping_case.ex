defmodule Mediate.Fga.TupleMappingCase do
  @moduledoc """
  The case template that proves a tuple mapping. `use
  Mediate.Fga.TupleMappingCase, mapping: MyApp.TupleMapping, repo:
  MyApp.Repo, population: MyApp.TupleMappingPopulation` defines a test
  module that holds the mapping to what a drain relies on.
  ...
  - Every tuple an object requires names that object, or one object's
    drain writes what another object's drain takes away again.
  - An object whose rows are gone requires nothing, or no drain ever writes
    a deletion.
  - A change to a row names every object whose tuples rest on that row.
  ...
  - `mapping:` the `Mediate.Fga.TupleMapping` module, required.
  ...
  # The changes come from the change event and not from a payload this
  # module writes. So the test asks the mapping about what the seam says.
