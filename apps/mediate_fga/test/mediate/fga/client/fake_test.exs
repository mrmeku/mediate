defmodule Mediate.Fga.Client.FakeTest do
  ...
  test "a call against a store that is not there is an engine error naming the call", context do
    assert {:error, %Error{reason: :engine_unreachable, detail: "Mediate.Fga failed during write_model" <> _rest}} =
             Fake.write_model(context.agent, "store-404", %{})

    assert {:error, %Error{reason: :engine_unreachable, detail: "Mediate.Fga failed during read" <> _rest}} =
             Fake.read(context.agent, "store-404", %Read{object_type: "folder"})

    assert {:error, %Error{reason: :engine_unreachable, detail: "Mediate.Fga failed during write" <> _rest}} =
