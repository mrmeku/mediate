defmodule Mediate.FgaTest.Guard do
  @moduledoc false
...
defmodule Mediate.FgaTest do
  ...
  test "a scope above the cap records limited and check still answers each row", context do
    ...
    {rule, %Decision{} = decision} = Mediate.scope(ann(), :read, :folder)
    ...
    kept = for id <- 1..1_000, Mediate.check(ann(), :read, {:folder, id}), do: {:folder, id}
  ...
  # A callback that cannot reach the engine answers the same error, and names itself in it.
