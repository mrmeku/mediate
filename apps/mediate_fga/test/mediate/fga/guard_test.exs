defmodule Mediate.Fga.GuardTest.Cleared do
...
defmodule Mediate.Fga.GuardTest do
  # What every callback relies on when it consults a guard. It answers a
  # boolean about every operation under every environment, and the same
  # answer twice. It admits nothing beyond what the test told it to admit,
  # because a guard that cannot tell is a guard that does not admit.
  use ExUnit.Case, async: true

  alias Mediate.Fga.GuardTest.Cleared

  @now ~U[2026-09-09 12:00:00.000000Z]
  @admitted %{now: @now, cleared: true}
  @refused %{now: @now, cleared: false}
  @undeclared :mediate_fga_guard_test_undeclared_operation
