defmodule Mediate.Fga.Infrastructure.DecideTest do
  ...
  test "an operation is a relation, a subject is a user, and an object is one of the store" do
    ...
    assert Decide.fallback_event() == [:mediate, :fga, :scope_fallback]
  end
  ...
  test "the entry is what the configuration names, and an entry naming less is an engine error", context do
    ...
    for field <- [:endpoint, :store_id] do
      ...
      assert error.detail ==
               "#{inspect(Mediate.Fga)} failed during decide: the configuration entry names no #{field}"
    end
  end
  ...
  test "a client without a client named is the client over HTTP", context do
    ...
    assert entry.client == Mediate.Fga.Client.Http
  end
  ...
  test "an entry that pins no model asks nothing at all", context do
    ...
    assert asked == "#{inspect(Mediate.Fga)} failed during decide: #{detail}"
  ...
  test "an engine error on the way to the server is the answer, whichever callback asked", context do
    ...
    assert {:error, %Error{reason: :engine_unreachable, detail: "Mediate.Fga failed during check" <> _rest}} =
             Decide.one(entry, ann(), :read, folder)

    assert {:error, %Error{reason: :engine_unreachable, detail: "Mediate.Fga failed during list_objects" <> _rest}} =
