defmodule Mediate.Fga.Relay.JobTest do
  # What a runner relies on of a job, held against the job this package
  # proves itself over. A read answers entries above the position it got,
  # in position order, and no more of them than the limit. Positions rise
  # and are unique, so a cursor that holds the last position of a batch
  # holds every position in it. A batch that arrived already can arrive
  # again, since the next pass reads a pass that does not commit again.
  ...
  # The job's own options as a runner hands them, with the defaults filled
  # in as `Mediate.Fga.Relay.Options` fills them in there.
