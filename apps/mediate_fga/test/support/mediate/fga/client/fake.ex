defmodule Mediate.Fga.Client.Fake do
  @moduledoc """
  The client behaviour on an `Agent`: stores, models, and tuples in one
  process. So a suite drives the drain and the adapter without a server. A
  test starts one of its own and passes the agent as the endpoint.

  What it copies from the server is what a caller can get wrong. A write is
  atomic per call. A call changes nothing at all when it carries one of
  these:

  - a duplicate write
  - a delete of a tuple the store does not hold
  - a tuple key on both of its sides
  - more changes than one call can carry

  The fake matches deletes and writes by the tuple key alone. So a tuple
  written again with another condition is a duplicate.

  What it does not copy is the model. `check/3` and `list_objects/3` answer
  from the tuples the store holds directly, in the types the real client
  answers in. They compose no relation and evaluate no condition. A
  decision is what a server does, and the cases that need one run against
  a server.

  `fail_after/2` makes the next writes fail once the fake has acknowledged
  a number of them. That is how a test interrupts a drain part way.
  """

  @behaviour Mediate.Fga.Client

  use Boundary, top_level?: true, deps: [Mediate, Mediate.Fga]
  use Agent
  ...
  @doc "Fails every write after this many more succeed. `nil` fails none."
  ...
  # The fake decides every refusal before it puts anything back. So a call
  # that one of its changes fails leaves the store as it was.
