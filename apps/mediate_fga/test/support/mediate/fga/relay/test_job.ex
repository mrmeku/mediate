defmodule Mediate.Fga.Relay.TestJob do
  @moduledoc """
  ...
  A repeat lands nothing, which is what a job built for at-least-once
  ...
  use Boundary, top_level?: true, deps: [Ecto, NimbleOptions, Mediate.Fga.Relay], exports: [Row, Sent]

  import Ecto.Query, only: [from: 2]

  alias Mediate.Fga.Relay.Entry
  alias Mediate.Fga.Relay.Job
  alias Mediate.Fga.Relay.TestJob.Row
  alias Mediate.Fga.Relay.TestJob.Sent
  ...
  @schema NimbleOptions.new!(
            ...
            tag: [type: :string, default: "sent", doc: "The tag a delivered row carries."]
          )
  ...
  def read(repo, options, from, limit) do
    ...
    {:ok, Enum.map(repo.all(query, mediate: @exemption), &%Entry{position: &1.position, payload: &1.body})}
  end
  ...
  def deliver(repo, options, entries) do
    ...
    options = [on_conflict: :nothing, conflict_target: [:runner, :position], mediate: @exemption]
    {_count, nil} = repo.insert_all(Sent, rows, options)

    :ok
  end

  @doc "Writes `count` rows into the default population, above every position already there."
  @spec write(module(), pos_integer()) :: :ok
  def write(repo, count), do: write(repo, @default, count)

  @doc "Writes `count` rows into one population, above every position already there."
  ...
  def write(repo, runner, count) when is_binary(runner) and count > 0 do
    ...
    {^count, nil} = repo.insert_all(Row, rows, mediate: @exemption)
  ...
  def arrived(repo, runner) when is_binary(runner) do
    ...
    repo.all(query, mediate: @exemption)
  end
end

defmodule Mediate.Fga.Relay.TestJob.Row do
  @moduledoc "A row on its way out, whose serial is the position that orders it."
  use Ecto.Schema

  @type t :: %__MODULE__{}

  schema "mediate_relay_test_rows" do
    field :runner, :string
    field :body, :string
  end
end

defmodule Mediate.Fga.Relay.TestJob.Sent do
  @moduledoc "A row that reached the far end, once per runner and position."
  use Ecto.Schema

  @type t :: %__MODULE__{}

  schema "mediate_relay_test_sent" do
    field :runner, :string
