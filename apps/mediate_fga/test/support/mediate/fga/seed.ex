defmodule Mediate.Fga.Seed do
  ...
  not a shortcut around it. Each object costs a read and the difference, so
  a world after a world writes only what changed.

  The seed marks the objects itself and leaves none of it to the change
  handler. The template counts the queries a write costs, and a handler
  that writes a marker inside that write is one more. What the handler
  does instead is `Mediate.Fga.OutboxCase`'s to hold.

  The fixture's items are rows the mapping states no tuple for. An item
  answers as its folder does, and the model reaches the folder through a
  ...
  mapping leaves out of `object_types/0`. That is why reconcile ignores
  them.
  """
  ...
  # Where the configured entry says the store is. That is the store the
