defmodule Mediate.Postgres.Infrastructure.Settings do
  @moduledoc false
  # The session settings a call runs under. A call always sets four:
  # `mediate.subject_id`, `mediate.subject_kind`, `mediate.operation`,
  # and `mediate.now`. Beyond those, every fact the caller supplied in the
  # environment gets a setting under its own name. So an application whose
  # policies read `current_setting('mediate.reauthenticated_at', true)`
  ...
  # equal to anything a policy grants on, so an absent fact denies.
  #
  # `statement/1` renders the settings as one `SELECT` over
  # `set_config(name, value, true)`: one statement, one round trip, and one
  # query in the shape counts. `hash/1` is the SHA-256 a scope decision
  # carries as its rule text, because under row-level security the
  # settings are what the database enforced. A build of the settings runs
  # no statement. This module decides what the settings are. The module
  # that holds the connection sets them.
  ...
  @doc """
  The settings for a subject, an operation, and the environment of the
  call. The second form takes the time of a decision in place of the
  environment, for a caller that holds a decision alone. It carries no
  supplied fact, so a policy that reads one denies.
  """
  @spec of(Mediate.subject(), atom(), Mediate.environment() | DateTime.t()) :: t()
  ...
  around it holds the connection. A policy reads a name at the empty string
  as "no operation in force". No statement can unset a name.
  """
  @spec cleared(t()) :: t()
