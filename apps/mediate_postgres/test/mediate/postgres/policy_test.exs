defmodule Mediate.Postgres.PolicyTest do
  ...
  test "the text a policy version carries names the table, the policy, and both expressions" do
    text = Policy.to_text(%{policy("mediate_scope_read") | using: "true", with_check: nil})
    assert text == "folders mediate_scope_read select\n  USING true\n  WITH CHECK -\n"
  end
