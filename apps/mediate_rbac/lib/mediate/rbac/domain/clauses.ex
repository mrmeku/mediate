defmodule Mediate.Rbac.Domain.Clauses do
  @moduledoc false
  # The clauses of a policy. `Mediate.Rbac.Policy` generates one call of
  ...
  # fails at its first read and not on a request. `Mediate.Rbac.Policy` says
  # why the check runs at read time and not at compile time.
  ...
  @predicate_schema NimbleOptions.new!(
                      only: [type: {:list, :atom}, doc: "The operations the predicate applies to, or all when absent."]
                    )
