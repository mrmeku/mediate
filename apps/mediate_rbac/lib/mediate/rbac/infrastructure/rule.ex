defmodule Mediate.Rbac.Infrastructure.Rule do
  @moduledoc false
  # A protected schema's rule for one subject and operation, built from the
  # policy's clauses as `dynamic` expressions over the protected row:
  #
  # - each grant is a membership test against a subquery of its
  #   relationship schema. One subquery per hop wraps it when the grant runs
  #   `through` other schemas
  # - each predicate that applies to the operation is the function's result
  # - the whole is any grant and every predicate
  #
  # A build runs no query. `dynamic/1` is the rule `scope` returns, and the
  # clause map is what the decider selects for the rows asked about. One
  # build serves both, so a scoped query and a checked row agree.
  ...
  @doc """
  Builds the rule. Answers the deny answer when the policy has no object
  of the type, or when no role permits the operation. Answers
  `{:error, detail}` when a predicate returned neither a `dynamic` nor a
  boolean.
  """
  @spec build(Policy.t(), Mediate.subject(), atom(), atom(), Mediate.environment()) ::
  ...
  # spans relationship schemas with different role columns fails to cast the
  # roles another schema's column holds.
