...
defmodule Mediate.Rbac.Policy.Clause do
  @moduledoc """
  One clause of a protected schema's rule.

  A grant names a relationship schema whose rows hold a role for the
  subject on the object:

  - `on` is the column of the protected schema the relationship's object
    column names
  - `role` is the relationship column that holds the role
  - `as` is a role every row holds, when the relationship has no role column
  - `through` is the chain of hops a grant crosses when the relationship
...
defmodule Mediate.Rbac.Policy.Object do
  @moduledoc "A protected schema and the clauses of its rule."

  alias Mediate.Rbac.Policy.Clause
...
defmodule Mediate.Rbac.Policy do
  ...
  The rule allows an operation on a row when any grant holds a role that
  ...
  `Mediate.Rbac.Policy.Clause` says what each option means. A predicate is
  ...
  alias Mediate.Rbac.Domain.Clauses
  ...
  @doc "Makes the module a policy. `use_schema/0` has the options."
  defmacro __using__(options) do
    quote bind_quoted: [options: options] do
      import Mediate.Rbac.Policy, only: [role: 2, object: 2, grant: 2, grant: 3, predicate: 2, predicate: 3]

      alias Mediate.Rbac.Policy

      @mediate_code_options NimbleOptions.validate!(options, Policy.use_schema())
      Module.register_attribute(__MODULE__, :mediate_code_roles, accumulate: true)
      Module.register_attribute(__MODULE__, :mediate_code_objects, accumulate: true)
  ...
  defmacro role(name, permissions) do
    quote bind_quoted: [name: name, permissions: permissions] do
      @mediate_code_roles Mediate.Rbac.Policy.__role__(name, permissions)
    end
  end
  ...
  defmacro object(schema, do: block) do
    quote do
      @mediate_code_clauses []
      unquote(block)
      @mediate_code_objects {unquote(expanded(schema, __CALLER__)), Enum.reverse(@mediate_code_clauses)}
    end
  end
  ...
  defmacro grant(name, source, options \\ []) do
    ...
    quote do
      @mediate_code_clauses [unquote(Macro.escape(clause)) | @mediate_code_clauses]
    end
  end
  ...
  defmacro __before_compile__(_env) do
    quote do
      @doc false
      @spec __mediate_code__(:roles | :objects | :options) :: term()
      def __mediate_code__(:roles), do: Enum.reverse(@mediate_code_roles)

      def __mediate_code__(:objects), do: unquote(__MODULE__.__objects__(__CALLER__.module))

      def __mediate_code__(:options), do: @mediate_code_options
    end
  end

  @doc "The role table."
  @spec roles(t()) :: [Role.t()]
  def roles(policy) when is_atom(policy), do: policy.__mediate_code__(:roles)

  @doc "The protected schemas and their clauses, built and checked against the schemas they name."
  @spec objects(t()) :: [Object.t()]
  def objects(policy) when is_atom(policy), do: policy.__mediate_code__(:objects)
  ...
  def options(policy) when is_atom(policy), do: policy.__mediate_code__(:options)

  @doc false
  @spec __objects__(module()) :: Macro.t()
  def __objects__(module) when is_atom(module) do
    objects = Enum.reverse(Module.get_attribute(module, :mediate_code_objects))
  ...
  # clause reads the same when the generated function compiles. The
  ...
  defp expanded(ast, env) do
    inside = %{env | function: {:__mediate_code__, 1}}
