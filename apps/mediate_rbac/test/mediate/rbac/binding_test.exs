defmodule Mediate.Rbac.BindingTest do
  ...
  alias Mediate.TestRepos.Committed
  ...
  test "without a binding every call is an engine error, so the port fails closed" do
    ...
    assert {:error, %Error{reason: :engine_unreachable, detail: detail}} =
             Mediate.Rbac.scope(ann, :read, :folder, environment, [])

    assert detail =~ "Mediate.Rbac failed during scope"
  end
  ...
  test "a policy that did not use Mediate.Rbac.Policy is invalid" do
    assert {:error, %Error{reason: :invalid, detail: detail}} = Binding.new(policy: Sandboxed, repo: Sandboxed)
    assert detail =~ "did not use Mediate.Rbac.Policy"
