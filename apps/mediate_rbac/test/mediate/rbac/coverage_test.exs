defmodule Mediate.Rbac.CoverageTest do
  ...
  defmodule Reads do
    @moduledoc false
    @spec name(Mediate.subject(), Mediate.Mediate.environment()) :: Ecto.Query.dynamic_expr()
    def name(_subject, _environment), do: dynamic([row], row.name == "public")

    @spec joined(Mediate.subject(), Mediate.Mediate.environment()) :: Ecto.Query.dynamic_expr()
    ...
    @spec raw(Mediate.subject(), Mediate.Mediate.environment()) :: Ecto.Query.dynamic_expr()
    def raw(_subject, _environment), do: dynamic([row], fragment("? = 'public'", row.name))

    @spec mapped(Mediate.subject(), Mediate.Mediate.environment()) :: Ecto.Query.dynamic_expr()
    ...
    @spec parent(Mediate.subject(), Mediate.Mediate.environment()) :: Ecto.Query.dynamic_expr()
    def parent(_subject, _environment), do: dynamic([row], not is_nil(row.folder_id))

    @spec grandparent(Mediate.subject(), Mediate.Mediate.environment()) :: Ecto.Query.dynamic_expr()
    def grandparent(_subject, _environment) do
      notes = from(n in Mediate.Rbac.CoverageTest.Note, where: not is_nil(n.folder_id), select: n.id)
  ...
  defmodule Note do
    ...
    schema "mediate_coverage_test_notes" do
  ...
  defmodule Remark do
    ...
    schema "mediate_coverage_test_remarks" do
  ...
  test "the coverage failure names the undeclared column and its schema" do
    ...
    assert_raise ArgumentError, ~r/name of Mediate.Fixture.Folder/, fn -> Coverage.check!(NamePolicy) end
  end
