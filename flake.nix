{
  description = "Mediate: the toolchain, the test cluster's Postgres, and the two policy engines";
  ...
  # Cerbos has no nixpkgs package. The flake fetches the release tarball below.
  inputs = {
    ...
    flake-parts.lib.mkFlake { inherit inputs; } {
      ...
      perSystem =
        ...
        let
          ...
          # Cerbos 0.55.0, the latest release at
          # https://api.github.com/repos/cerbos/cerbos/releases/latest on
          # 2026-09-08, published 2026-08-13. The hashes come from the
          # downloaded assets through `openssl dgst -sha256 -binary | base64`.
          # On macOS the binary comes from a tarball and not from a notarised
          # application bundle. So it carries no quarantine attribute, and
          # Gatekeeper runs it with no prompt.
          cerbos =
            let
              version = "0.55.0";
              assets = {
        ...
        in
        {
          ...
          # exploration by hand. The test suite never uses them. It starts a
          # cluster of its own.
          process-compose."services" = {
            ...
            services.postgres."postgres" = {
              ...
              initialScript.before = ''
                CREATE ROLE mediate_owner LOGIN;
                CREATE ROLE mediate_app LOGIN NOBYPASSRLS;
              '';
              initialDatabases = [ { name = "mediate_dev"; } ];
              # The owner role owns the development database, as it owns each
              # database of the test cluster. So a migration it runs can create
              # in the public schema, and the tables it creates are its own.
              initialScript.after = ''
                ALTER DATABASE mediate_dev OWNER TO mediate_owner;
              '';
            };
            settings.processes = {
              cerbos.command = ''
