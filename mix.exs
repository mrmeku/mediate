defmodule Mediate.Umbrella.MixProject do
  use Mix.Project
  ...
  # The tools every application shares. Each pin is exact, and mix.lock
  # holds the checksum. Versions verified against
  ...
  # `mix test` at the root, and the test step of the alias, run each
  # application's suite in an operating-system process of its own. The
  # umbrella's own recursion starts every application in one VM before the
  # first suite runs. The thin applications bind the same example modules,
  # the repos and the audit store among them, so one VM cannot hold two of
  # them. A process per application also gives each suite a cluster of its
  # own. Arguments pass through to each application's run.
  ...
  # measures coverage over a part of the suite. So this run has no
  # partitions and runs on its own, over the applications that own a
  # `domain/`. It reads them from the tree and not from a list kept by hand.
  defp run_domain_tests(args) do
    System.put_env("MEDIATE_DOMAIN_COVERAGE", "1")
